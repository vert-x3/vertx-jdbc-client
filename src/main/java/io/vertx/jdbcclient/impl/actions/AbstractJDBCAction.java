...
public abstract class AbstractJDBCAction<T> implements JDBCAction<T> {
  ...
  protected final PrepareOptions prepareOptions;
  ...
  protected AbstractJDBCAction(JDBCStatementHelper helper, SqlOptions options) {
    this(helper, options, null);
  }

  protected AbstractJDBCAction(JDBCStatementHelper helper, SqlOptions options, PrepareOptions prepareOptions) {
  ...
  protected boolean isAutoGeneratedKeys() {
    // PrepareOptions take precedence over connection-level SqlOptions.
    if (prepareOptions != null) {
      JDBCPrepareOptions jdbcPrepareOptions = JDBCPrepareOptions.createFrom(prepareOptions);
      return jdbcPrepareOptions.isAutoGeneratedKeys();
    }
    return options == null || options.isAutoGeneratedKeys();
  }

  protected JsonArray getAutoGeneratedKeysIndexes() {
    // PrepareOptions take precedence over connection-level SqlOptions.
    if (prepareOptions != null) {
      JDBCPrepareOptions jdbcPrepareOptions = JDBCPrepareOptions.createFrom(prepareOptions);
      return jdbcPrepareOptions.getAutoGeneratedKeysIndexes();
    }
    return options != null ? options.getAutoGeneratedKeysIndexes() : null;
  }
