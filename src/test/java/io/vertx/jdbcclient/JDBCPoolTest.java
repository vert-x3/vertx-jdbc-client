...
//          should.assertNull(row.getBuffer(0));
  ...
  @Test
  public void testBatchWithEnabledGeneratedKeys(TestContext should) {
    final Async test = should.async();
    ...
    // Create prepare options with auto-generated keys explicitly enabled
    JDBCPrepareOptions options = new JDBCPrepareOptions()
    ...
    client
      ...
      .onSuccess(rows -> {
        should.assertNotNull(rows);
        should.assertEquals(2, rows.size());
        // Generated keys should be available (if database supports it)
        Row keys = rows.property(JDBCPool.GENERATED_KEYS);
  ...
  @Test
  public void testPrepareOptionsOverridesConnectionOptions(TestContext should) {
    final Async test = should.async();

    // Create a connection with auto-generated keys enabled by default
    JDBCConnectOptions connectOpts = connectOptions()
      .setAutoGeneratedKeys(true);

    Pool poolWithKeys = JDBCPool.pool(vertx, connectOpts, poolOptions());

    String sql = "INSERT INTO insert_table (fname, lname) VALUES (?, ?)";
    List<Tuple> batch = Arrays.asList(
      Tuple.of("frank", "farmer")
    );

    // Override connection-level setting with prepare options
    JDBCPrepareOptions options = new JDBCPrepareOptions()
      .setAutoGeneratedKeys(false);
